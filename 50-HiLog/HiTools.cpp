...
string HiTools::GetFileVersion(string filePath) {
...
BOOL HiTools::FileExist(std::string file) {
	struct stat buffer;
	return (stat(file.c_str(), &buffer) == 0);
}

wstring HiTools::itow(int i) {
	wstring data;
	wchar_t t[10];
	wmemset(t, 0, 10);
	wsprintf(t, TEXT("%d"), i);
...
string HiTools::w2s(wstring wstr) {
	string result;
...
string HiTools::wstring2string(wstring wstr) {
	return w2s(wstr);
}

wstring HiTools::s2w(string str) {
	wstring result;
...
wstring HiTools::string2wstring(string str) {
	return s2w(str);
}

const char* HiTools::u2g(string str) {
...
string& HiTools::Replace(string& str, const string& old_value, const string& new_value) {
...
string HiTools::Base64_Encode(const char* data, int len) {
...
string HiTools::Base64_Encode(string str) {
	return Base64_Encode(str.c_str(), str.size());
}

string HiTools::Base64_Decode(const char* data, int len, int& olen) {
...
string HiTools::Base64_Decode(string data, int& olen) {
	return Base64_Decode(data.c_str(), data.size(), olen);
}

std::wstring HiTools::RegReadView(HKEY root, LPCWSTR path, LPCWSTR key, DWORD type) {
	wstring value(TEXT(""));
	HKEY hKey;
...
DWORD HiTools::ReadInt(DWORD address) {
	DWORD intValue = 0;
	intValue = *(DWORD*)address;
	return intValue;
}

string HiTools::ReadAsciiString(DWORD address) {
	string sValue = "";
...
wstring HiTools::ReadUnicodeString(DWORD address) {
	wstring value;
