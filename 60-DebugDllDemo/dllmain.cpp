...
void SetRoomName(wstring roomid, wstring roomname);
...
INT_PTR CALLBACK DialogProc(_In_ HWND   hwndDlg, _In_ UINT   uMsg, _In_ WPARAM wParam, _In_ LPARAM lParam)
{
    ...
    case WM_COMMAND:
        ...
        if (wParam == IDOK)
        {
            ...
            wstring roomid(wRoomId);
            wstring roomname(wGroupName);
...
void SetRoomName(wstring roomid, wstring roomname) {
    ...
    DWORD dwCallAddr1 = wxBaseAddress + WxSetRoomName;

    GeneralStruct RoomWxid = { 0 };
    RoomWxid.pstr = (wchar_t*)roomid.c_str();
    RoomWxid.iLen = roomid.size();
    RoomWxid.iMaxLen = roomid.capacity();

    GeneralStruct RoomName = { 0 };
    RoomName.pstr = (wchar_t*)roomname.c_str();
    RoomName.iLen = roomname.size();
    RoomName.iMaxLen = roomname.capacity();
